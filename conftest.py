...
import numpy as np
import pytest
from multipose2 import utils, models, vit_sam
import zipfile
import torch
...
class MockTransformer(vit_sam.Transformer):
    def __init__(self, use_layers: int, in_channels: int = 3,
        ...
        super().__init__(in_channels=in_channels, adapter_type=adapter_type)
...
class MockCellposeModel(models.CellposeModel):
    def __init__(self, n_keep_layers=2, gpu=False, nchan=3,
                 adapter_type="linear"):
        super().__init__(gpu=gpu, nchan=nchan, adapter_type=adapter_type)
